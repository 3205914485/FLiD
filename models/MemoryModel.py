...
class MemoryModel(torch.nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
        ...
        self.message_dim = self.memory_dim + self.memory_dim + \
            self.time_feat_dim + self.edge_feat_dim
        ...
        self.memory_bank = MemoryBank(
            num_nodes=self.num_nodes, memory_dim=self.memory_dim)
        if self.model_name == 'TGN':
            ...
            raise ValueError(
                f'Not implemented error for model_name {self.model_name}!')

        # embedding module
        if self.model_name == 'JODIE':
            self.embedding_module = TimeProjectionEmbedding(
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        if self.model_name == 'JODIE':
            ...
            src_node_time_intervals = torch.from_numpy(node_interact_times).float().to(
                self.device) - updated_node_last_updated_times[torch.from_numpy(src_node_ids)]
            src_node_time_intervals = (
                src_node_time_intervals - self.src_node_mean_time_shift) / self.src_node_std_time_shift
            # Tensor, shape (batch_size, )
            dst_node_time_intervals = torch.from_numpy(node_interact_times).float().to(
                self.device) - updated_node_last_updated_times[torch.from_numpy(dst_node_ids)]
        ...
        src_node_embeddings, dst_node_embeddings = node_embeddings[:len(
            src_node_ids)], node_embeddings[len(src_node_ids): len(src_node_ids) + len(dst_node_ids)]
        ...
        if self.model_name == 'DyRep':
            src_node_embeddings = updated_node_memories[torch.from_numpy(
                src_node_ids)]
            dst_node_embeddings = updated_node_memories[torch.from_numpy(
                dst_node_ids)]
    ...
    def compute_new_node_raw_messages(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, dst_node_embeddings: torch.Tensor,
        ...
        src_node_memories = self.memory_bank.get_memories(
        ...
        if self.model_name == 'DyRep':
            ...
            dst_node_memories = self.memory_bank.get_memories(
                node_ids=dst_node_ids)

        # Tensor, shape (batch_size, )
        src_node_delta_times = torch.from_numpy(node_interact_times).float().to(self.device) - \
            self.memory_bank.node_last_updated_times[torch.from_numpy(
                src_node_ids)]
    ...
    def set_neighbor_sampler(self, neighbor_sampler: NeighborSampler):
        ...
        assert self.model_name in [
            'TGN', 'DyRep'], f'Neighbor sampler is not defined in model {self.model_name}!'
    ...
    def aggregate_messages(self, node_ids: np.ndarray, node_raw_messages: dict):
        ...
        for node_id in unique_node_ids:
            if len(node_raw_messages[node_id]) > 0:
                ...
                unique_node_timestamps.append(
                    node_raw_messages[node_id][-1][1])
...
class MemoryBank(nn.Module):
    ...
    def __init__(self, num_nodes: int, memory_dim: int):
        ...
        self.node_memories = nn.Parameter(torch.zeros(
            (self.num_nodes, self.memory_dim)), requires_grad=False)
    ...
    def backup_memory_bank(self):
        ...
        for node_id, node_raw_messages in self.node_raw_messages.items():
            cloned_node_raw_messages[node_id] = [(node_raw_message[0].clone(
            ), node_raw_message[1].copy()) for node_raw_message in node_raw_messages]
    ...
    def reload_memory_bank(self, backup_memory_bank: tuple):
        ...
        :return:
        """
        self.node_memories.data, self.node_last_updated_times.data = backup_memory_bank[0].clone(
        ), backup_memory_bank[1].clone()

        self.node_raw_messages = defaultdict(list)
        for node_id, node_raw_messages in backup_memory_bank[2].items():
            self.node_raw_messages[node_id] = [(node_raw_message[0].clone(
            ), node_raw_message[1].copy()) for node_raw_message in node_raw_messages]
    ...
    def store_node_raw_messages(self, node_ids: np.ndarray, new_node_raw_messages: dict):
        ...
        for node_id in node_ids:
            self.node_raw_messages[node_id].extend(
                new_node_raw_messages[node_id])
...
class MemoryUpdater(nn.Module):
    ...
    def update_memories(self, unique_node_ids: np.ndarray, unique_node_messages: torch.Tensor,
        ...
        # Tensor, shape (num_unique_node_ids, memory_dim)
        updated_node_memories = self.memory_updater(
            unique_node_messages, node_memories)
        # update memories for nodes in unique_node_ids
...
class GRUMemoryUpdater(MemoryUpdater):
    ...
    def __init__(self, memory_bank: MemoryBank, message_dim: int, memory_dim: int):
        ...
        self.memory_updater = nn.GRUCell(
            input_size=message_dim, hidden_size=memory_dim)
...
class TimeProjectionEmbedding(nn.Module):
    ...
    def compute_node_temporal_embeddings(self, node_memories: torch.Tensor, node_ids: np.ndarray, node_time_intervals: torch.Tensor):
        ...
        """
        # Tensor, shape (batch_size, memory_dim)
        source_embeddings = self.dropout(node_memories[torch.from_numpy(
            node_ids)] * (1 + self.linear_layer(node_time_intervals.unsqueeze(dim=1))))
...
class GraphAttentionEmbedding(nn.Module):
    ...
    def compute_node_temporal_embeddings(self, node_memories: torch.Tensor, node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        node_features = node_memories[torch.from_numpy(
            node_ids)] + self.node_raw_features[torch.from_numpy(node_ids)]
        ...
        if current_layer_num == 0:
            ...
            output = self.merge_layers[current_layer_num -
                                       1](input_1=output, input_2=node_features)
...
def compute_src_dst_node_time_shifts(src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray):
    ...
    for k in range(len(src_node_ids)):
        ...
        src_node_all_time_shifts.append(
            node_interact_time - src_node_last_timestamps[src_node_id])
        dst_node_all_time_shifts.append(
            node_interact_time - dst_node_last_timestamps[dst_node_id])
