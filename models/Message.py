...
class Message(nn.Module):
    def __init__(self,num_events:int,device:str,updater:str='gru',message_funciton:str='te',dim_messages:int=172,dim_events:int=172,windows_size:int=1,encoders:int=1,dff:int=512,drop_out:float=0.1):
        ...
        self.window=[]
        #if updater=='gru':
        self.updater=nn.GRUCell(input_size=dim_messages,hidden_size=dim_messages)
        self.message_function=get_message_function(module_type=message_funciton,raw_message_dimension=dim_events,message_dimension=dim_messages,num_att_heads=4,hidden_size=256,encoders=encoders,dff=dff,drop_out=drop_out)
        self.net=torch.zeros([self.num_events,self.dim_messages],device=self.device)
