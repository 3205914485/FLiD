...
def evaluate_model_node_classification_temc(model_name: str, model: nn.Module, dataset: str, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader, offest: int,
...
def Temc_train(Dirtrainer: Trainer, gt_weight, data, pseudo_labels, pseudo_labels_store, args, logger, iter_num):

    double_way_datasets = args.double_way_datasets
    logger.info(f"Starting Temc-train\n")
    ...
    consistency_loss_fn = nn.MSELoss(reduction='mean')
    save_model_name = f'dir_{model_name}'
    save_model_folder = f"./saved_models/temc/{args.prefix}/{args.dataset_name}/{args.seed}/{save_model_name}/"
    ...
    for epoch in range(args.num_epochs_temc):
        model.train()
        node2label_history = {}
        ...
        try:
            ...
            print(f"tqdm init failed: {e}")
            train_idx_data_loader_tqdm = train_idx_data_loader  # fallback
        ...
        # get predicted probabilities, shape (batch_size, )
            ...
            consistency_losses = []
            for idx, node_id in enumerate(batch_src_node_ids):
                if node_id.item() in node2label_history:
                    prev_pred = node2label_history[node_id.item()]
                    curr_pred = predicts[idx].detach()
                    consistency_losses.append(consistency_loss_fn(curr_pred, prev_pred.to(curr_pred.device)))
                node2label_history[node_id.item()] = predicts[idx].detach()

            if consistency_losses:
                loss_consistency = torch.stack(consistency_losses).mean()
            else:
                loss_consistency = torch.tensor(0.0, device=predicts.device)

            loss = loss_gt + (1 - gt_weight) * loss_ps + args.consistency_weight * loss_consistency
        ...
        val_total_loss, val_metrics, val_metrics_gt = evaluate_model_node_classification_temc(model_name=model_name,
                                                                                                model=model,
        ...
        if (epoch + 1) % args.test_interval_epochs == 0:
            ...
            test_total_loss, test_metrics, test_metrics_gt = evaluate_model_node_classification_temc(model_name=model_name,
                                                                                                       model=model,
                                                                                                       dataset=args.dataset_name,
                                                                                                       neighbor_sampler=full_neighbor_sampler,
    ...
    if model_name not in ['TGN']:
        val_total_loss, val_metric, val_metrics_gt = evaluate_model_node_classification_temc(model_name=model_name,
                                                                                               model=model,
                                                                                               dataset=args.dataset_name,
                                                                                               neighbor_sampler=full_neighbor_sampler,
                                                                                               evaluate_idx_data_loader=val_idx_data_loader,
    ...
    idx_data_loader_tqdm = full_idx_data_loader
