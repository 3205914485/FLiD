...
def em_warmup(args, data, logger, Etrainer: Trainer, Mtrainer: Trainer, src_node_embeddings, dst_node_embeddings, pseudo_labels):
    ...
    new_src_embeddings, new_dst_embeddings = link_prediction(args=args, Etrainer=Etrainer, data=data, logger=logger)
    ...
    if args.warmup_e_train:
        for epoch in range(args.num_epochs_e_warmup):
            ...
            if (epoch + 1) % args.test_interval_epochs == 0:
                ...
                for metric_name in test_metrics[0].keys():
                    test_metric_indicator.append((metric_name, np.mean([test_metric[metric_name] for test_metric in test_metrics]), True))
