...
def em_warmup(args, data, logger, Mtrainer: Trainer, Etrainer: Trainer, src_node_embeddings, dst_node_embeddings, pseudo_labels, pseudo_labels_store):
    ...
    new_src_embeddings, new_dst_embeddings = link_prediction(
        args=args, Mtrainer=Mtrainer, data=data, logger=logger)
...
def link_prediction(args, Mtrainer, data, logger):
    ...
    model = nn.Sequential(Mtrainer.model, link_predictor)
